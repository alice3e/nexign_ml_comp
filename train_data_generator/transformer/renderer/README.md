...
Этот модуль использует **bpmn-js** и **Playwright** для высококачественного рендеринга BPMN диаграмм в PNG изображения.

**Важно:** Используется Playwright вместо Puppeteer для лучшей совместимости с Apple Silicon (M1/M2/M3).
...
Это установит:
- **playwright** - для рендеринга в headless браузере (лучше работает на Apple Silicon)
- **bpmn-js** (загружается через CDN в HTML)

После установки нужно скачать браузер Chromium:
```bash
...
Playwright требует дополнительных системных библиотек:
