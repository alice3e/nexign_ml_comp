...
- Случайными шрифтами и размерами
...
2. **Важно для корректного отображения текста:**

Рендерер автоматически ищет системные шрифты с поддержкой Unicode/кириллицы. Убедитесь, что в системе установлен хотя бы один из следующих шрифтов:
...
# Fedora/RHEL
sudo dnf install dejavu-sans-fonts liberation-fonts noto-fonts
```

**macOS:**
Системные шрифты уже поддерживают кириллицу (Helvetica, Arial).

**Windows:**
Системные шрифты уже поддерживают кириллицу (Arial, Calibri).

Если шрифты не найдены, рендерер использует встроенный шрифт PIL (ограниченная поддержка символов).
...
- **Task**: Прямоугольник со скругленными углами
- **StartEvent**: Круг с тонкой обводкой
- **EndEvent**: Круг с толстой обводкой
- **ExclusiveGateway**: Ромб с X внутри
- **ParallelGateway**: Ромб с + внутри
- **SequenceFlow**: Линия со стрелкой
